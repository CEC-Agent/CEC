...
def prepare_sample(
    ...
    for k, v in obs.items():
        if v.ndim == 4 and v.shape[-1] == 3:
            obs[k] = rearrange(v, "L H W C -> L C H W").astype(np.float32) / img_scale
    return obs, action
...
def collate_fn(
    ...
    padded_obs = padded_obs.to_torch_tensor(
        dtypes=torch.float16 if fp16 else torch.float32
    )
    if isinstance(padded_action, dict):
        ...
        padded_action = U.any_to_torch_tensor(padded_action).to(
    ...
    padded_action_mask = U.any_slice(
        padded_action_mask, np.s_[..., 0]
    )  # reduce action dim
