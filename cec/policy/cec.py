...
from cec.policy.utils import make_action_head


class CECPolicy(LightningModule, BasePolicy):
    def __init__(
        self,
        ...
        self.pi_head = make_action_head(
            action_space=action_space, input_dim=hidden_size, **pi_head_kwargs
        )

    @staticmethod
    def _prepare_input(obs, state, first_mask, acs=None):
        if acs is not None:
            obs["action"] = acs
    ...
    def forward(self, *args, **kwargs):
        ...
        x, prompt = self.feature_encoders(obs, first_mask=first_mask)
        x = self.feature_linear(x)

        x, state_out = self.transformer(
            x=x, first=first_mask, state=state, prompt=prompt
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i, (obs, action, action_mask) in enumerate(
            ...
            prev_actions = (
                ...
                if "action" in self.feature_encoders
                else None
            )
            pi, state = self.forward(obs, state, first_mask, acs=prev_actions)
