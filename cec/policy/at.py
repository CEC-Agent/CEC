...
class ATPolicy(LightningModule, BasePolicy):
    def __init__(self, config, img_encoder):
        super().__init__()
        self.at = AT(config, img_encoder)
    ...
    def training_step(self, batch, batch_idx):
        ...
        dtokens = obs_dict["d_token"].long()
        B, L = states.shape[:2]
        timesteps = (
            torch.arange(L, device=self.device).unsqueeze(0).long()
        )  # (1, L), broadcastable
        action_preds = self.forward(
            states, actions, returns_to_go, rewards, dtokens, timesteps, pad_masks
        )  # (B, L, A), logits
        ...
        acc = ((action_preds.argmax(-1) == actions) * pad_masks).sum() / pad_masks.sum()
    ...
    def act(self, *args, deterministic: bool, **kwargs):
        with torch.no_grad():
            logits, past_kv = self.at.act(*args, **kwargs)  # (B, 1)
...
class AT(DecisionTransformerPreTrainedModel):
    """
    ...
    def __init__(self, config, img_encoder):
        ...
        self.embed_return_to_go = torch.nn.Linear(1, config.hidden_size)
        ...
        self.embed_dtoken = torch.nn.Embedding(2, config.hidden_size)
    ...
    def forward(
        ...
        return_to_go_embeddings = self.embed_return_to_go(returns_to_go)
        reward_embeddings = self.embed_reward(rewards)
        dtoken_embeddings = self.embed_dtoken(dtokens)
        time_embeddings = self.embed_timestep(timesteps)

        # time embeddings are treated similar to positional embeddings
        state_embeddings = state_embeddings + time_embeddings
        action_embeddings = action_embeddings + time_embeddings
        return_to_go_embeddings = return_to_go_embeddings + time_embeddings
        reward_embeddings = reward_embeddings + time_embeddings
        dtoken_embeddings = dtoken_embeddings + time_embeddings
    ...
    def act(
        ...
        input_seq = []
        return_to_go_embedding = self.embed_return_to_go(return_to_go)
        ...
        if prev_action is not None:
            ...
            prev_reward_embedding = self.embed_reward(prev_reward)
            prev_dtoken_embedding = self.embed_dtoken(prev_dtoken)
            prev_time_embedding = self.embed_timestep(timestep - 1)
            prev_action_embedding = prev_action_embedding + prev_time_embedding
            prev_reward_embedding = prev_reward_embedding + prev_time_embedding
            prev_dtoken_embedding = prev_dtoken_embedding + prev_time_embedding
            input_seq.append(prev_action_embedding)
            input_seq.append(prev_reward_embedding)
            input_seq.append(prev_dtoken_embedding)
